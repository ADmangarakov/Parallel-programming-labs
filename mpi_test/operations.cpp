...
double calc_norm(double const *const vec, int const N, int const rank, int const size)
{
	double norm = 0;
	...
	FILE *f;
	char filename[32];
	snprintf(filename, sizeof(char) * 32, "whole_vec_proc%i.txt", rank);
	f = fopen(filename, "a");
	...
	int const rank, int const size)
{
	...
	double answ_norm = calc_norm(answ, N, rank, size);
	double *const buff = mul_vec_by_matr(vec, matr, N, rank, size);
	subtract_vectors(buff, answ, N);
	double norm = calc_norm(buff, N, rank, size) / answ_norm;
	FILE *f;
	char filename[32];
	snprintf(filename, sizeof(char) * 32, "whole_vec_norm%i.txt", rank);
	f = fopen(filename, "a");
	fprintf(f, "norm is: %f \n", norm);
	fprintf(f, "\n");
	fclose(f);
