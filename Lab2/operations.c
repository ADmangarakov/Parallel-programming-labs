//
// Created by User on 014 14.04.20.
//
...
double *mul_vec_by_matr(double const *const vector, double const *const matrix, int const N, int const iteration) {
...
void mul_vec_by_numb(double *const vector, double const number, int const N) {
    double local_number = number;
#pragma omp parallel for
    for (int i = 0; i < N; i++) {
        vector[i] *= local_number;
    }
}
...
double approximate(double *vector, double const *const matrix, double const *const answers,
                   int const N, double const t, int const iteration) {
    double *buff;
...
int is_answer_correct(double const sq_sum, double const *const answ, int const N, int const iteration) {
    double const static E = 1e-3;
    double answ_norm = 0;
#pragma omp parallel for reduction(+:answ_norm)
    for (int i = 0; i < N; i++) {
